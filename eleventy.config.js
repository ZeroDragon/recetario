...
/**
 * Shortcode pareado `flujo`: convierte una mini-sintaxis de receta en la tabla
...
// ---------------------------------------------------------------------------
// Parser: mini-DSL -> fases con filas (cada fila declara sus etapas en orden)
// ---------------------------------------------------------------------------

function renderFlujo(raw) {
  ...
  return phases.map(renderPhase).join("\n");
}
...
function renderPhase(ph) {
  ...
  for (let i = 0; i < filas.length; i++) {
    ...
    cells += f.cant ? `<strong>${esc(f.cant)}</strong><br />${esc(f.nombre)}` : esc(f.nombre);
    cells += "</td>";
